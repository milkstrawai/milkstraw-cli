// tests/unit/commands/commitments-rds.test.ts
...
import { listRdsCommitments } from '../../../src/services/commitments/rds.js';
...
describe('commitments rds list', () => {
  ...
  it('renders text table and forwards filters', async () => {
    vi.mocked(listRdsCommitments).mockResolvedValue({
      items: [sample],
    ...
    expect(listRdsCommitments).toHaveBeenCalledWith('tok', 'org_1', {
      accounts: ['1'],
      regions: ['us-east-1'],
      states: ['active'],
    });
    expect(result).toContain('rds-ri-1');
  ...
  it('returns slim quiet projection including state', async () => {
    ...
    expect(JSON.parse(result)).toEqual([
      { id: 'rds-ri-1', accountId: '928751460984', region: 'us-east-1', state: 'active' },
    ]);
  });
});
